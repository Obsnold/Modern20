...
#
#   tools/deploy.sh [user@host] [--packs] [--overwrite-live]
#
...
No host to deploy to. Either pass one:
    ...
    tools/deploy.sh user@host --packs
...
python3 tools/check_globals.py
python3 tools/check_app_props.py
...
python3 tools/check_rules_links.py
python3 tools/check_capture.py
python3 tools/check_art.py
python3 tools/check_deploy.py
python3 tools/check_selftest.py
python3 tools/gen_pregens.py --check
python3 tools/gen_guide.py --check
python3 tools/gen_cover.py --check
python3 tools/gen_scene.py --check
...
if [ -n "$PACKS" ] && [ -z "$OVERWRITE" ]; then
  ...
  if ! python3 tools/capture_edits.py "$HOST" --dry-run --exit-code; then
    cat >&2 <<'MESSAGE'

The live compendia hold something src/packs does not. Packing would overwrite
it. Bring it home first:

    python3 tools/capture_edits.py       # then read the diff and commit

or, if the live packs really are the ones to throw away:

    tools/deploy.sh --packs --overwrite-live
MESSAGE
    exit 1
  fi
fi
...
node tools/check_casting.mjs
node tools/check_combat.mjs
node tools/check_creatures.mjs
