...
"""Build the six ready-made characters, one per basic class.
    ...
    python3 tools/gen_pregens.py            # write src/packs/pregens
    python3 tools/gen_pregens.py --check    # fail if they are out of date
...
def build_character(character: dict) -> tuple[dict, list[str]]:
    ...
    if not actor["img"]:
        problems.append("has no artwork; tools/art.py has no rule for it")
    ...
    for actor in documents:
        ...
        if arguments.check:
            print(f"FAIL  src/packs/pregens/{actor['_slug']}.json no longer matches "
                  "data/pregens.json — run python3 tools/gen_pregens.py")
            return 1
