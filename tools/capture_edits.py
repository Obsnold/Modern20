...
deliberately differ from the SRD, and why.
    ...
    tools/capture_edits.py                     # pull from the Foundry host
    tools/capture_edits.py --from /tmp/src     # compare an unpacked copy
    tools/capture_edits.py --dry-run           # say what changed, write nothing
    tools/capture_edits.py --dry-run --exit-code   # fail if anything has
...
def main() -> int:
    ...
    if not source and not args.host:
        print("No host to read from. Pass one, or set MODERN20_HOST:\n"
              "    python3 tools/capture_edits.py user@host\n"
        ...
        for pack in sorted(os.listdir(source)):
            ...
            if overrides and not args.dry_run:
                ...
                existing.setdefault("_comment",
                                    ...
                                    "captured from Foundry by tools/capture_edits.py. "
