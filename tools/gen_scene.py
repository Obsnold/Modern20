...
"""Draw a scene to play on, and the walls that go with it.
    ...
    python3 tools/gen_scene.py            # write the map and src/packs/scenes
    python3 tools/gen_scene.py --check    # fail if either is out of date
...
def main() -> int:
    ...
    if arguments.check:
        for name in stale:
            print(f"FAIL  {name} is out of date — run python3 tools/gen_scene.py")
        if stale:
            return 1
