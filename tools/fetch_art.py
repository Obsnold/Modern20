#!/usr/bin/env python3
"""Vendor the icons `tools/art.py` names, from game-icons.net.

    python3 tools/fetch_art.py            # fetch what is missing
    python3 tools/fetch_art.py --force    # fetch everything again
    python3 tools/fetch_art.py --list     # say what would be fetched
...
def credits(used: dict[str, str]) -> str:
    ...
    lines = [
        ...
        "by `tools/fetch_art.py`, which also generates this file.",
        "",
        "## By author",
        "",
    ]
...
def main() -> int:
    ...
    if arguments.list:
        print(f"{len(wanted)} icon(s) named by tools/art.py:")
        for icon in wanted:
            print(f"  {icon}")
        return 0
