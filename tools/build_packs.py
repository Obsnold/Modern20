...
"""Turn scraped SRD tables into Foundry compendium source documents.
    ...
    python3 tools/scrape.py        # writes data/*.json
    python3 tools/build_packs.py   # writes src/packs/<pack>/<slug>.json
...
def apply_document_override(document: dict, override: dict) -> None:
    ...
    what tools/capture_edits.py writes - names where each value goes:
...
def main() -> int:
    ...
    packs = build()
    if not packs:
        print("No packs built. Run tools/scrape.py first.", file=sys.stderr)
        return 1
